...
export default async function About() {
  ...
  const initialData = await mockAsyncFn(3000)

  const finalData = Array.from({ length: 150 }, async() =>
    await mockAsyncFn(Math.random() * 20 * 1000)
  );

  return (
    <div className="m-5 font-mono text-green-400 flex-col justify-center ">
      <div className="text-center m-10">Initial Load time: {initialData}</div>
